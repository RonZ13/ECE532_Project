 ...
 */
volatile int KeyboardInitFin = FALSE; /* Flag to indicate Init sequence finished */
...
volatile int keyboard_inputs[NUM_KB_I_FIFO];
volatile int keyboard_inputs_head = 0;
volatile int keyboard_inputs_tail = 0;
	...
	switch (Index) {
		...
		default : {
			if (KeyInst.Break == FALSE){
				int idx = Index+(85*!((KeyInst.RShift || KeyInst.LShift) != (KeyInst.CapsLock)));
				if ((keyboard_inputs_tail + 1) % NUM_KB_I_FIFO != keyboard_inputs_head){
					// xil_printf(keys[idx]);
					keyboard_inputs[keyboard_inputs_tail] = idx;
					keyboard_inputs_tail++;
					keyboard_inputs_tail %= NUM_KB_I_FIFO;
				} else {
					xil_printf("Keyboard input FIFO Overflow !!!");
				}
			}
			//F1..F12
		}
...
u8 KeyboardInit (void){
	...
	// xil_printf ("1\n");
	// setting all the keyboard status LEDs to off
	while(1){
		RxDataRecv = FALSE;
	...
	// xil_printf ("3\n");
	// reading the device ID to confirm it is a keyboard
	while(1){
		RxDataRecv = FALSE;
	...
	// xil_printf ("4\n");
	// setting all the status LEDs to on
	while(1){
		RxDataRecv = FALSE;
		axi_ps2_SendByte(Ps2Inst.Ps2Config.BaseAddress, 0xED);
		if(KbWaitReply(Ps2Ack)){
	...
	// xil_printf ("6\n");
	// setting all the status LEDs to off
	while(1){
		RxDataRecv = FALSE;
		axi_ps2_SendByte(Ps2Inst.Ps2Config.BaseAddress, 0xED);
		if(KbWaitReply(Ps2Ack)){
	...
	// xil_printf ("8\n");
	// setting the typematic rate to 5 character per second
	while(1){
		RxDataRecv = FALSE;
	...
	// xil_printf ("11\n");
	// setting the typematic rate to 30 character per second
	while(1){
		RxDataRecv = FALSE;
		axi_ps2_SendByte(Ps2Inst.Ps2Config.BaseAddress, 0xF3);
		if(KbWaitReply(Ps2Ack)){
	...
	while(1){
		RxDataRecv = FALSE;
		axi_ps2_SendByte(Ps2Inst.Ps2Config.BaseAddress, 0x00);
		if(KbWaitReply(Ps2Ack)){
			KeyInst.KbInitStage++;
			break;
		}
...
//		xil_printf ("\r\nBytesSent");
	...
	if (RxDataRecv == TRUE && KeyboardInitFin == TRUE) {
		KbMain();
	}
}
