	...
	while(1){
		...
		if (!is_pause){
			...
			if (cnt%200 == 0){
				...
				printLyrics(front, 10, 340, LYRICS_VGA_WINDOW_LEN, LYRICS_INPUT_BUFF_SIZE, font, lettercolor, lyrics, &lyrics_input_head, &lyrics_output_head, &lyrics_output_tail, 0);
			}
			usleep(1000);
		}
			...
			if (strcmp(character, "\r\n") == 0){
				switch (state){
					...
					case pause:
						...
						break;
					case replay:
						printLyrics(front, 10, 340, LYRICS_VGA_WINDOW_LEN, LYRICS_INPUT_BUFF_SIZE, font, lettercolor, lyrics, &lyrics_input_head, &lyrics_output_head, &lyrics_output_tail, 1);
						break;
					default:
